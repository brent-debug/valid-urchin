    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <input
          ...
          className="flex-1 min-w-0 px-3 py-2 rounded-lg border border-zinc-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary-600"
        />
        <select
          value={timeRange.label}
          onChange={e => setTimeRange(TIME_RANGES.find(t => t.label === e.target.value))}
          className="px-3 py-2 rounded-lg border border-zinc-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary-600"
        >
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {Object.entries(groups).map(([url, items]) => {
            ...
            return (
              <div key={url} className="bg-white rounded-xl border border-zinc-200 overflow-hidden">
                <button
